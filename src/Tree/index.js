...
export default class Tree extends React.Component {
  ...
  findActiveNodes(nodeId, nodeSet, hits) {
    if (hits.length > 0) {
      return hits;
    }

    hits = hits.concat(nodeSet.filter(node => node.nodeSvgShape.shapeProps.fill === 'blue'));

    nodeSet.forEach(node => {
      if (node._children && node._children.length > 0) {
        hits = this.findActiveNodes(nodeId, node._children, hits);
      }
    });
   ...
   * @return {void}
   */
  setActive(node, activeNodes) {
    if (activeNodes.length > 0) {
      activeNodes.forEach(activeNode => {
        if (activeNode.name === 'Eligible') {
          activeNode.nodeSvgShape.shapeProps.fill = 'green';
        } else if (activeNode.name === 'Not Eligible') {
          activeNode.nodeSvgShape.shapeProps.fill = 'red';
        } else if (activeNode.name === 'Eligibility Undetermined') {
          activeNode.nodeSvgShape.shapeProps.fill = 'orange';
        } else {
          activeNode.nodeSvgShape.shapeProps.fill = 'grey';
        }
      });
      node.nodeSvgShape.shapeProps.fill = 'blue';
    } else {
      node.nodeSvgShape.shapeProps.fill = 'blue';
    }
  }
  ...
  handleNodeToggle = (nodeId, evt) => {
    console.log('NO I AM CALLED');
    const data = clone(this.state.data);
    const matches = this.findNodesById(nodeId, data, []);
    const activeNodes = this.findActiveNodes(nodeId, data, []);
    const targetNode = matches[0];

    // Persist the SyntheticEvent for downstream handling by users.
    evt.persist();

    this.setActive(targetNode, activeNodes);
